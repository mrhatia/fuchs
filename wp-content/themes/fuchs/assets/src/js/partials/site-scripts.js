	...
	if ( jQuery( '.hero-home' ).length ) {
		...
		function showSlide( index ) {
			...
			if ( index >= slides.length ) {
				index = 0;
			} // go to first if past last
			if ( index < 0 ) {
				index = slides.length - 1;
			} // go to last if before first
			current = index;

			// incoming slide always starts from left side
			gsap.set( slides.eq( current ), { x: '-100%' } );
